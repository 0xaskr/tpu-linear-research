...
def kda_gate_chunk_cumsum_vector_kernel(
    ...
    b_A = tl.load(A_log + i_h).to(tl.float32)
    if not USE_LOWER_BOUND:
        # 1. 并没有算全局 Gate，也没有全局同步。
        #    这里计算的是 **每个时间步独立 (Per-Token) 的瞬时遗忘门值** (Instantaneous Log-Decay)。
        #    它的计算只依赖于当前的输入 b_s 和全局参数 b_A，因此是完全并行的 (Element-wise)。
        #    只有后面的 tl.cumsum 才会引入依赖，但也局限在 Chunk 内部。
        b_gate = -exp(b_A) * softplus(b_s)
    else:
        # 下界保护模式：确保 log-decay 不会小于 lower_bound (如 -5.0)，防止梯度消失或过度遗忘。
        b_gate = lower_bound * tl.sigmoid(exp(b_A) * b_s)

    # Apply chunk local cumsum
    # 这一步计算 块内的局部累加门控值 (Local Log-Augmentations)。
    # 数学含义：KDA 中的 gate 本质上是 log-space 的衰减系数 (decay)。
    # 为了计算不同时间步之间的影响（例如 t 时刻的信息流向 t+k 时刻），
    # 我们需要连乘每一部的衰减： D = \prod_{i=t}^{t+k} decay_i
    # 在对数空间里，连乘变成累加： log(D) = \sum_{i=t}^{t+k} log(decay_i)
    # 这里的 b_gate 就是 log(decay)，tl.cumsum 就是在计算累加衰减。
    if REVERSE:
...
def kda_gate_chunk_cumsum(
    ...
    **kwargs,
    ...
    B, T, H, S = g.shape
    BT = chunk_size
    # 这里的计算是条件性的 (Conditional Computation)，仅作为兜底逻辑。
    # 1. 灵活性：允许此函数独立调用，调用者只需提供 cu_seqlens 即可，无需手动准备复杂的 indices。
    # 2. 也是为了与上层模块解耦：如果上层已经算好了 indices 传进来，这里就会跳过 (if chunk_indices is None)。
    # 3. 性能保障：即使进入此分支，prepare_chunk_indices 有 @tensor_cache 缓存机制，
    #    只要 cu_seqlens 张量对象没变，开销极小。
