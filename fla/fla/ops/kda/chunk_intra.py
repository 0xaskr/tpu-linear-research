...
def chunk_kda_fwd_intra(
    ...
    r"""
    KDA (Kimi Delta Attention) 的块内计算 (Intra-chunk computation)。

    此函数计算块内注意力矩阵和衰减矩阵，并求解每个块内由 Delta Rule 递归隐含的线性系统。

    数学公式:
    1. 注意力分数矩阵 (Aqk):
       编码带有数据依赖衰减的 Query-Key 交互。
       A_qk[t, s] = scale * (q[t] @ k[s]^T) * exp(g[t] - g[s]), 其中 t >= s

    2. Key-Key 相关性矩阵 (Akk):
       编码 Delta Rule 更新中 Key 之间的依赖关系。
       A_kk[t, s] = beta[t] * (k[t] @ k[s]^T) * exp(g[t] - g[s]), 其中 t > s

    3. 逆矩阵 / 求解 (Inverse / Solve):
       该函数计算下三角系统 (I + A_kk) 的逆，从而获得正交化 Key（或计算 'w'）所需的变换。
       A_kk_inv = (I + strict_tril(A_kk))^-1

    参数:
        q (torch.Tensor): Queries [B, T, H, K]
        k (torch.Tensor): Keys [B, T, H, K]
        v (torch.Tensor): Values [B, T, H, V] (用于重计算)
        gk (torch.Tensor): Log-decay gates [B, T, H, K]
        beta (torch.Tensor): Beta 系数 [B, T, H]
    ...
    # Step 1: 计算对角线块 (Diagonal Blocks) 并求局部逆
    # 数学含义:
    #   1. 计算子块内部的 Aqk 和 Akk:
    #      Aqk_{sub} = q_{sub} @ k_{sub}^T * decay
    #      Akk_{sub} = k_{sub} @ k_{sub}^T * decay * beta
    #   2. 对每个子块的 Akk 进行原地求逆 (In-place Inversion):
    #      Akk_{sub, inv} = (I + StrictlyTril(Akk_{sub}))^{-1}
    #   这一步确保了对角线上的块已经是逆矩阵的一部分。
    if safe_gate:
        grid = (NT, NC, B * H)
    ...
    # Step 2: 计算非对角块并完成全块求逆 (Global Chunk Inverse)
    # 数学含义:
    #   1. 计算子块之间的 Akk 和 Aqk (Off-diagonal blocks)。
    #   2. 使用块状前向代入 (Block Forward Substitution) 求解整个 Chunk 的逆矩阵。
    #      如果我们将大矩阵看作块状下三角矩阵:
    #      M = [[A, 0], [B, C]]
    #      M^{-1} = [[A^{-1}, 0], [-C^{-1} B A^{-1}, C^{-1}]]
    #      这里的 Kernel 迭代地执行这个过程，最终得到全块的 Akk_inv。
    grid = (NT, B * H)
    ...
    # Step 3: 重计算正交化后的键值 w 和 u (Recompute w, u)
    # 数学含义:
    #   利用前面算出的 Akk_inv (即 (I + Akk)^{-1}) 来计算 Delta Rule 的有效投影。
    #   w = (I + Akk)^{-1} @ k
    #   u = 这里的 u 通常指代用于最终 Attention 的值向量变换 (具体取决于 implementation detail)
