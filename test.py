...
def load(pointer, mask=None, other=None, boundary_check=(), padding_option="", cache_modifier="", eviction_policy="",
    ...
    返回一个张量，其值是从 `pointer` 定义的内存位置加载的：
        ...
        (2) 如果 `pointer` 是 N 维指针张量，则加载 N 维张量。在这种情况下：
            ...
            - `mask` 和 `other` 隐式广播到 `pointer.shape`，
            - `other` is implicitly typecast to `pointer.dtype.element_ty`, and
            - `other` 隐式转换为 `pointer.dtype.element_ty`，并且
            - `boundary_check` and `padding_option` must be empty.
            - `boundary_check` 和 `padding_option` 必须为空。
        ...
        (3) 如果 `pointer` 是由 `make_block_ptr` 定义的块指针，则加载张量。在这种情况下：
    ...
    :param mask: if `mask[idx]` is false, do not load the data at address `pointer[idx]`
        (must be `None` with block pointers)
                 如果 `mask[idx]` 为 false，则不加载地址 `pointer[idx]` 处的数据（对于块指针必须为 `None`）
    :type mask: Block of `triton.int1`, optional
    :param other: if `mask[idx]` is false, return `other[idx]`
                  如果 `mask[idx]` 为 false，则返回 `other[idx]`
    ...
    :param padding_option: should be one of {"", "zero", "nan"}, the padding value to use while out of bounds. "" means an undefined value.
                           应该是 {"", "zero", "nan"} 之一，越界时使用的填充值。"" 表示未定义的值。
    :param cache_modifier: changes cache option in NVIDIA PTX
                           更改 NVIDIA PTX 中的缓存选项
    ...
    :param eviction_policy: changes eviction policy in NVIDIA PTX
                            更改 NVIDIA PTX 中的驱逐策略
    :type eviction_policy: str, optional
    :param volatile: changes volatile option in NVIDIA PTX
                     更改 NVIDIA PTX 中的 volatile 选项
...
def store(pointer, value, mask=None, boundary_check=(), cache_modifier="", eviction_policy="", _semantic=None):
    ...
    将数据张量存储到由 `pointer` 定义的内存位置。
        ...
        (1) If `pointer` is a single element pointer, a scalar is stored.  In
            this case:
        (1) 如果 `pointer` 是单个元素的指针，则存储标量。在这种情况下：
        ...
        (2) 如果 `pointer` 是 N 维指针张量，则存储 N 维块。在这种情况下：

            - `mask` is implicitly broadcast to `pointer.shape`, and
            - `mask` 隐式广播到 `pointer.shape`，并且
        ...
        (3) 如果 `pointer` 是由 `make_block_ptr` 定义的块指针，则存储数据块。在这种情况下：
    ...
    `value` 隐式广播到 `pointer.shape` 并转换为 `pointer.dtype.element_ty`。
    ...
    :param mask: If `mask[idx]` is false, do not store `value[idx]` at `pointer[idx]`
                 如果 `mask[idx]` 为 false，则不在 `pointer[idx]` 处存储 `value[idx]`
    :type mask: Block of triton.int1, optional
    :param boundary_check: tuple of integers, indicating the dimensions which should do the boundary check
                           整数元组，指示应进行边界检查的维度
    :type boundary_check: tuple of ints, optional
